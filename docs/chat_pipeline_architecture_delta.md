...
- SENTRY routing runs before the main model path and records `FAST_PATH`, `DEEP_PATH`, or `SAFETY_ESCALATION`.
...
- Cache invalidation now fires from client check-ins, logged workouts, client profile/why/memory edits, trainer client memory edits, and trainer queue approvals that affect a client.
...
## Interface Changes
- New env vars: `REDIS_URL`, `CHAT_CACHE_ENABLED`, `CHAT_CACHE_TIMEOUT_MS`, `CHAT_ROUTER_TIMEOUT_MS`, `CHAT_STREAM_LEGACY_ALIAS_ENABLED`.
