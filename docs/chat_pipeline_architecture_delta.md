...
- Timeout and partial-failure paths now degrade predictably: router failure falls back to `DEEP_PATH`, streaming Postgres timeouts emit a minimal safe response, LLM creation retries use the configured retry budget, memory writes retry once, pre-open/first-byte/no-commit stream failures retry the provider fallback chain, and post-commit mid-stream LLM failures emit structured SSE errors.
- Intelligence jobs are visible at enqueue time through `intelligence_jobs` and the `worker_queue_lag` view; `/healthz` reports queue lag and dead-letter state.
...
- `backend/sql/20260516a_worker_queue_lag_view.sql`
- `backend/scripts/chat_load_baseline.py`
- `backend/tests/test_chat_pipeline_primitives.py`

## Interface Changes
- This blocker-fix pass adds no new external API routes or env vars.
...
- New SQL migration adds `worker_queue_lag` and queue lifecycle states for enqueue-time visibility.

## Migration Steps
1. Apply `backend/sql/20260511_add_conversation_metadata.sql`.
2. Apply `backend/sql/20260516a_worker_queue_lag_view.sql`.
3. Configure `REDIS_URL` for production chat caches.
4. Keep `CHAT_STREAM_LEGACY_ALIAS_ENABLED=true` until deployed clients all support canonical `token` events.

## Launch Gate Evidence
- Load scripts are generated in `backend/scripts/chat_load_baseline.py`.
- Human-run staging results should be saved under `docs/load_test_results/YYYY-MM-DD.md` using the template in `docs/load_test_results/README.md`.
- Results are not yet recorded in this repo; staging operator must run the five load tests against the current Render concurrency settings before launch.
