...
| `REDIS_URL` | Redis backend for worker queues, chat cache, and production request rate limiting. | Redis/cache provider secret manager. | local optional; staging/prod required for release checks. | Yes | No | `<redis://cache-host:6379/0>` |
| `CHAT_CACHE_ENABLED` | Enables chat Redis helper when `REDIS_URL` is present. | Backend runtime config. | local optional; staging/prod recommended `true`. | No | No | `<true>` |
| `CHAT_CACHE_TIMEOUT_MS` | Socket timeout for chat cache operations. | Backend runtime config. | local/prod default `25`; staging launch gate uses `150` to tolerate managed Redis p95 latency. | No | No | `<150>` |
| `CHAT_ROUTER_TIMEOUT_MS` | Target budget for SENTRY route classification. | Backend runtime config. | local/staging/prod default `200`. | No | No | `<200>` |
| `CHAT_STREAM_LEGACY_ALIAS_ENABLED` | Emits temporary `message_delta` alias after canonical `token` events. | Backend runtime config. | keep `true` until all deployed clients consume `token/content`. | No | No | `<true>` |
| `MAX_ACTIVE_CHAT_STREAMS_PER_INSTANCE` | Per-process active `/api/v1/chat/stream` semaphore limit. | Backend runtime config. | staging default `15`; production default `10`, validated safe cap per launch gate 2026-05-17; launch breach validation used staging `25` with one uvicorn worker. Raise to `15+` pending provider tier audit. | No | No | `<10>` |
| `USE_FAKE_PROVIDER` | Switches chat stream smoke paths to fake provider output. | Backend runtime config. | must be `false` for real-provider launch load validation. | No | No | `<false>` |
...
| `EXPO_PUBLIC_SUPABASE_REDIRECT_URL` | Redirect URI used by auth/deep-link security checks. | App auth config. | local recommended; staging recommended; prod recommended. | No | Yes | `<ai.modefit.app://auth/callback>` |
