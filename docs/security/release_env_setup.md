...
| `MAX_ACTIVE_CHAT_STREAMS_PER_INSTANCE` | Per-process active `/api/v1/chat/stream` semaphore limit. | Backend runtime config. | staging default `15`; production default `25`; launch breach validation uses staging `25` with one uvicorn worker. | No | No | `<25>` |
