...
MODEL_ROUTING = {
    ...
    "DEEP_PATH": {"provider": "openai", "model": GPT_5_5_MODEL, "tier": "full"},
    "SAFETY_ESCALATION": {"provider": "openai", "model": GPT_5_5_MODEL, "tier": "full"},
...
def provider_fallback_chain(route: RoutingDecision) -> list[ProviderAttempt]:
    ...
    is_deep_path = route_name == "DEEP_PATH" or route.flow in {"deep_path", "reasoning_structured"}
...
def estimate_cost_usd(model: str, prompt_tokens: int, completion_tokens: int) -> float | None:
    return calculate_cost_usd(model, prompt_tokens, completion_tokens)
